...
class User(db.Model, UserMixin):
    ...
    favorites = db.relationship('FavoritePlan', back_populates='user', cascade='all, delete-orphan')
...
class MonetaryRechargePlan(db.Model):
    ...
    # Relationships
    favorites = db.relationship('FavoritePlan', back_populates='monetaryPlan', cascade='all, delete-orphan')
...
class MobileDataPlan(db.Model):
    ...
    # Relationships
    favorites = db.relationship('FavoritePlan', back_populates='dataPlan', cascade='all, delete-orphan')


# this table will store the plans thatthe user choose as favorite
class FavoritePlan(db.Model):
    __tablename__ = 'favoritePlan'  
    id = db.Column(db.Integer, primary_key=True)
    userId = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)  # Foreign key to the User table
    monetaryPlanId = db.Column(db.Integer, db.ForeignKey('monetaryRechargePlan.id'), nullable=True) 
    dataPlanId = db.Column(db.Integer, db.ForeignKey('mobileDataPlan.id'), nullable=True) 

    # Relationships
    user = db.relationship('User', back_populates='favorites')
    monetaryPlan = db.relationship('MonetaryRechargePlan', back_populates='favorites')
    dataPlan = db.relationship('MobileDataPlan', back_populates='favorites')
